...
//Modifer: Junho Kim
// moved rmTrigVersionFromName into each Class 
// to avoid compile error regarding multiple definition 
// NEED TO UNDERSTAND WHY THIS ERROR OCCURED EVEN THE PREPROCESSOR #ifndef IS USED
...
public:
  ...
  void fill(const boost::property_tree::ptree& psTbl){
    ...
    pathData.push_back(val);
      ...
      Entry tblEntry;
      tblEntry.name = rmTrigVersionFromName(pathData[1]);
...
private:
  ...
  std::string jsonBaseName_;
public:  
  
  void setPrescaleProvider(const std::string& jsonBaseName = "dummy"){

    jsonBaseName_ = jsonBaseName;

    boost::property_tree::ptree runInfoTree,l1PSTree,hltPSTree;
    boost::property_tree::read_json(jsonBaseName_+"_runInfo.json",runInfoTree);
    boost::property_tree::read_json(jsonBaseName_+"_l1prescales.json",l1PSTree);
    boost::property_tree::read_json(jsonBaseName_+"_hltprescales.json",hltPSTree);
  ...
  std::string rmTrigVersionFromName(std::string pathName) const{
    size_t versionIndex = pathName.rfind("_v");
    if(versionIndex!=std::string::npos) pathName.erase(versionIndex+2);
    return pathName;
  }
  ...
  int l1Prescale(const std::string& l1Seed,int runnr,int lumiSec)const{
    ...
    if(runInfo) return runInfo->l1PrescaleFromLumiSec(rmTrigVersionFromName(l1Seed),lumiSec);
    else return -1;
  }
  int hltPrescale(const std::string& hltPath,int runnr,int lumiSec)const{
    auto runInfo = getRunInfo(runnr);
    if(runInfo) return runInfo->hltPrescaleFromLumiSec(rmTrigVersionFromName(hltPath),lumiSec);
    else return -1;
  }
