...
std::vector<Muon> AnalyzerCore::GetAllMuons(bool apply_roc){
  ...
  for(unsigned int i=0; i<muon_pt->size(); i++){
    ...
    if(apply_roc) mu.SetPtEtaPhiM(muon_pt->at(i)*rc, muon_eta->at(i), muon_phi->at(i), muon_mass->at(i));
...
bool AnalyzerCore::PassMETFilter(){
  ...
  //if(!Flag_BadChargedCandidateFilter) return false; // TODO 19/05/04 twiki says this in under review, and not recommended
