    ...
    if(PassMETFilter()){ 
        ...
        if(IsMuMu || IsElEl){
            ...
            if( leps.size() == 2 ){ // leps have leptons passing ID and trigger passed
                ...
                if(IsMuMu == 1){
                    if(fabs(leps.at(0)->Eta()) < LepEtaCut && fabs(leps.at(1)->Eta()) < LepEtaCut) evt_tag_leptoneta_sel_rec = 1;
                }
                if(IsElEl == 1){
                    if(fabs(leps.at(0)->Eta()) < LepEtaCut && fabs(leps.at(1)->Eta()) < LepEtaCut){
                            if((fabs(leps.at(0)->Eta()) < 1.4442 || fabs(leps.at(0)->Eta()) > 1.566) && (fabs(leps.at(1)->Eta()) < 1.4442 || fabs(leps.at(1)->Eta()) > 1.566)) evt_tag_leptoneta_sel_rec = 1;
                    }
                }
